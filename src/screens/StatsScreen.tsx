...
function statTile(n: string | number, label: string, bg: string, col: string, icon: string) {
  return (
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }} edges={['top']}>
      <ScrollView contentContainerStyle={{ padding: 18, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        ...
        {loading ? (
          ...
          <>
            <View style={s.tiles}>
              {statTile(data?.streak ?? 0, 'streak nay', '#FFF0E6', colors.coralDark, 'flamefill')}
              {statTile(data?.best_streak ?? 0, 'streak dài nhất', '#FFEAF2', colors.pinkDark, 'flame')}
              {statTile(data?.total_done ?? 0, 'tổng việc', '#EAF7F1', colors.mintDark, 'check')}
              {statTile(data?.active_days ?? 0, 'ngày chăm', '#EEE7FF', colors.purpleDark, 'calendar')}
            </View>
                  ...
                  return (
                    <View key={d.ymd} style={s.barCol}>
                      <View style={{ height: CHART_H, justifyContent: 'flex-end', width: '100%', alignItems: 'center' }}>
                        <View style={[
                          s.bar,
                          { height: h, backgroundColor: isToday ? colors.pink : d.count > 0 ? colors.purple : '#E7E0F2', width: range === 7 ? 22 : '68%' },
                        ]} />
                      </View>
                      <Text style={[s.barLbl, isToday && { color: colors.pinkDark, fontFamily: fonts.heading }]}>
                        {showLabel ? d.dd : ''}
                      </Text>
                    </View>
                  );
...
const s = StyleSheet.create({
  ...
  empty: { backgroundColor: '#F6ECFB', borderRadius: 18, padding: 18, marginTop: 16, borderWidth: 2, borderColor: '#fff' },
